...
public class MonoLibForge {
    ...
    public MonoLibForge() {
        ...
        MonoLibForge.EVENT_BUS.addListener((Consumer<RegisterEvent>) event -> {
            if (event.getRegistryKey().equals(Registries.COMMAND_ARGUMENT_TYPE)) event.<ArgumentTypeInfo<?, ?>>register(Registries.COMMAND_ARGUMENT_TYPE, new ResourceLocation(Constants.MOD_ID, "item_output"), () -> ArgumentTypeInfos.registerByClass(HandArgument.class, HandArgument.SERIALIZER));
        });
